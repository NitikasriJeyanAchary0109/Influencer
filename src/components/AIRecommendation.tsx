...
import { Sparkles, UserPlus, Check, Plus, Star, Percent, TrendingUp, Compass, Mail, Phone, Instagram } from 'lucide-react'
...
export const AIRecommendation: React.FC<Props> = ({ onInfluencerRecruited }) => {
  ...
  useEffect(() => {
    const fetchExisting = async () => {
      if (!brand) return
      const { data } = await supabase.from('influencers').select('instagram_handle').eq('brand_id', brand.brand_id)
      if (data) {
        setRecruitedHandles(data.map((i: any) => i.instagram_handle))
      }
    }
    fetchExisting()
  }, [brand])
