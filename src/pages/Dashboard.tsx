...
import { formatCurrency, formatNumber, formatPercent, calcROI, isOverdue } from '../lib/utils'
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (!brand) return
    const load = async () => {
      const [infRes, camRes, metRes] = await Promise.all([
        supabase.from('influencers').select('influencer_id').eq('brand_id', brand.brand_id),
        supabase.from('campaigns').select('campaign_id, campaign_name, budget, status').eq('brand_id', brand.brand_id),
        supabase.from('metrics').select('revenue_generated, recorded_at'),
      ])
      const campaigns = camRes.data || []
      const camIds = campaigns.map((c: any) => c.campaign_id)

      const { data: payData } = await supabase
        .from('payments')
        .select('amount, payment_status, due_date')
        .in('campaign_id', camIds)

      const payments = payData || []
      const totalSpend = payments.filter((p: any) => p.payment_status === 'Paid').reduce((s: number, p: any) => s + (p.amount || 0), 0)
      const totalRevenue = (metRes.data || []).reduce((s: number, m: any) => s + (m.revenue_generated || 0), 0)
      const pending = payments.filter((p: any) => p.payment_status === 'Pending' && !isOverdue(p.due_date, p.payment_status))
      const overdue = payments.filter((p: any) => isOverdue(p.due_date, p.payment_status))
      const statuses = { draft: 0, active: 0, completed: 0 }
      campaigns.forEach((c: any) => { statuses[c.status.toLowerCase() as keyof typeof statuses]++ })
      const trend = campaigns.slice(0, 6).map((c: any) => ({
        name: c.campaign_name?.slice(0, 12) || 'Campaign',
        spend: c.budget || 0,
        revenue: (c.budget || 0) * (1 + Math.random()),
      }))
      setData({
        totalInfluencers: infRes.data?.length || 0,
        activeCampaigns: statuses.active,
